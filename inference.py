...
def encode_binary_mask(mask: np.ndarray) -> str:
    ...
    # check input mask -- (Updated np.bool to bool for NumPy 1.20+
    # compatibility)
    if mask.dtype != bool:
        raise ValueError(
...
def process_tta_predictions(model, image: np.ndarray, img_size: int = 1024):
    ...
    for view_name, img_view in views.items():
        ...
        # Geometrically reverse the bounding boxes and masks to align with the
        # original view
        if view_name == "hflip":
...
def process_predictions(model, image: np.ndarray, img_size: int = 1024):
    ...
    if results_orig.boxes is not None and results_orig.masks is not None:
        ...
        masks_resized = np.array([cv2.resize(
            m, (img_w, img_h), interpolation=cv2.INTER_LINEAR) > 0.5 for m in masks])
        all_masks.append(masks_resized)
...
def main():
    ...
    if os.path.exists(jsonl_path):
        with open(jsonl_path, 'r') as f:
            for line in f:
                ...
                gloms = [
                    anno for anno in data.get(
                        'annotations',
                        []) if anno.get('type') == 'glomerulus']
                if gloms:
                    glom_annotations[data['id']] = gloms
    else:
        print(
            f"Warning: Metadata file not found at {jsonl_path}. Subtraction step will be bypassed.")
    ...
    for img_path in test_images:
        ...
        # Optimization: Pre-filter arrays before WBF to prevent OOM scaling
        # issues
        f_boxes_list, f_scores_list, f_labels_list, f_masks_list = [], [], [], []
        for b, s, l, m in zip(boxes_list, scores_list,
        ...
        if sum(len(b) for b in f_boxes_list) == 0:
            submission_data.append(
                {"id": img_id, "height": img_h, "width": img_w, "prediction_string": ""})
            continue
        ...
        for f_box, f_score, f_label in zip(
            ...
            if best_mask_idx != -1:
                ...
                # but omitted here during inference as it's not a target
                # evaluation class.
        ...
        if img_id in glom_annotations:
            for anno in glom_annotations[img_id]:
                for poly in anno['coordinates']:
                    ...
                    # Rasterize ground-truth polygons perfectly onto coordinate
                    # space
        ...
        for vessel_mask, score in final_vessels:
            ...
            # Application of 7th Place methodology: Remove small noise
            # fragments
            ...
            # iterations=1 is critical. More iterations will aggressively
            # delete valid micro-capillaries.
            opened_mask = cv2.morphologyEx(
                mask_uint8, cv2.MORPH_OPEN, kernel, iterations=1)

            # 3. Convert back to boolean for RLE encoding
            final_binary_mask = opened_mask > 127
            # --------------------------------------------------------------

            # Safety check: if the opening operation entirely deleted a faint
            # vessel, skip it
